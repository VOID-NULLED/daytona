...
type SessionWriteInputRequest struct {
	Input string `json:"input" validate:"required"`
} // @name SessionWriteInputRequest
...
func ToCommandDTO(c *session.Command) *CommandDTO {
	return &CommandDTO{
		Id:       c.Id,
		Command:  c.Command,
		ExitCode: c.ExitCode,
...
func ToSessionDTO(s *session.Session) *SessionDTO {
	...
	for _, cmd := range s.Commands {
		commands = append(commands, ToCommandDTO(cmd))
	}
